...
def relatorio(ini):
    ...
    params.append(datetime(1999, 1, 1))
    ...
    report_contas_pagar_techcd = pandas.read_sql_query(query_contas_pagar_techcd, connection, params=params)
    report_contas_pagar_data_store = pandas.read_sql_query(query_contas_pagar_data_store, connection, params=params)
    report_contas_pagar_midia_center = pandas.read_sql_query(query_contas_pagar_midia_center, connection, params=params)
    ...
    report_contas_despesas_techcd = pandas.read_sql_query(query_despesas_techcd, connection, params=params)
    report_contas_despesas_data_store = pandas.read_sql_query(query_despesas_data_store, connection, params=params)
    report_contas_despesas_midia_center = pandas.read_sql_query(query_despesas_midia_center, connection, params=params)
    ...
    message['Subject'] = "INVENTAIO CONTAS A PAGAR , PAGAS E DESPESAS  REFERENTE Á %s ATÉ %s" % \
                         (params[0].date().__format__('%d/%m/%y'), params[1].date().__format__('%d/%m/%y'))
...
if __name__ == '__main__':
    ...
    ini = datetime(today.year, _month, 1)
    relatorio(ini)
